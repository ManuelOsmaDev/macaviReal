  ...
  return (
    <div>
      <h2 id="cliente-heading" data-cy="ClienteHeading">
        <Translate contentKey="macaviApp.cliente.home.title">Clientes</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="macaviApp.cliente.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/cliente/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="macaviApp.cliente.home.createLabel">Create new Cliente</Translate>
          </Link>
        </div>
      </h2>
        ...
        {clienteList && clienteList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('razonSocial')}>
                  <Translate contentKey="macaviApp.cliente.razonSocial">Razon Social</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th className="hand" onClick={sort('telefono')}>
                  <Translate contentKey="macaviApp.cliente.telefono">Telefono</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="macaviApp.cliente.locate">Locate</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="macaviApp.cliente.tipoDni">Tipo Dni</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {clienteList.map((cliente, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{cliente.locate ? <Link to={`/ciudad/${cliente.locate.id}`}>{cliente.locate.id}</Link> : ''}</td>
                  <td>{cliente.tipoDni ? <Link to={`/tipo-dni/${cliente.tipoDni.id}`}>{cliente.tipoDni.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="macaviApp.cliente.home.notFound">No Clientes found</Translate>
            </div>
          )
        )}
      </div>
