...
class ClienteResourceIT {
    ...
    public static Cliente createEntity(EntityManager em) {
        Cliente cliente = new Cliente().razonSocial(DEFAULT_RAZON_SOCIAL).direccion(DEFAULT_DIRECCION).telefono(DEFAULT_TELEFONO);
        // Add required entity
        Ciudad ciudad;
        ...
        cliente.setLocate(ciudad);
        // Add required entity
        TipoDni tipoDni;
        if (TestUtil.findAll(em, TipoDni.class).isEmpty()) {
            tipoDni = TipoDniResourceIT.createEntity(em);
            em.persist(tipoDni);
            em.flush();
        } else {
            tipoDni = TestUtil.findAll(em, TipoDni.class).get(0);
        }
        cliente.setTipoDni(tipoDni);
        return cliente;
    }
    ...
    public static Cliente createUpdatedEntity(EntityManager em) {
        Cliente cliente = new Cliente().razonSocial(UPDATED_RAZON_SOCIAL).direccion(UPDATED_DIRECCION).telefono(UPDATED_TELEFONO);
        // Add required entity
        Ciudad ciudad;
        if (TestUtil.findAll(em, Ciudad.class).isEmpty()) {
            ciudad = CiudadResourceIT.createUpdatedEntity(em);
            em.persist(ciudad);
            em.flush();
        } else {
            ciudad = TestUtil.findAll(em, Ciudad.class).get(0);
        ...
        TipoDni tipoDni;
        if (TestUtil.findAll(em, TipoDni.class).isEmpty()) {
            tipoDni = TipoDniResourceIT.createUpdatedEntity(em);
            em.persist(tipoDni);
            em.flush();
        } else {
            tipoDni = TestUtil.findAll(em, TipoDni.class).get(0);
    ...
    void putNewCliente() throws Exception {
        ...
        updatedCliente.razonSocial(UPDATED_RAZON_SOCIAL).direccion(UPDATED_DIRECCION).telefono(UPDATED_TELEFONO);
    ...
    void fullUpdateClienteWithPatch() throws Exception {
        ...
        partialUpdatedCliente.razonSocial(UPDATED_RAZON_SOCIAL).direccion(UPDATED_DIRECCION).telefono(UPDATED_TELEFONO);
