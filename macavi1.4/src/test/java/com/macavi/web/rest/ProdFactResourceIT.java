...
class ProdFactResourceIT {
    ...
    public static ProdFact createEntity(EntityManager em) {
        ProdFact prodFact = new ProdFact().cantidad(DEFAULT_CANTIDAD);
        // Add required entity
        Factura factura;
    ...
    public static ProdFact createUpdatedEntity(EntityManager em) {
        ProdFact prodFact = new ProdFact().cantidad(UPDATED_CANTIDAD);
        // Add required entity
        Factura factura;
        if (TestUtil.findAll(em, Factura.class).isEmpty()) {
    ...
    void createProdFact() throws Exception {
        ...
        assertThat(testProdFact.getCantidad()).isEqualTo(DEFAULT_CANTIDAD);
    }
    ...
    void checkCantidadIsRequired() throws Exception {
        int databaseSizeBeforeTest = prodFactRepository.findAll().size();
        // set the field null
        prodFact.setCantidad(null);

        // Create the ProdFact, which fails.
        ProdFactDTO prodFactDTO = prodFactMapper.toDto(prodFact);
        ...
        List<ProdFact> prodFactList = prodFactRepository.findAll();
        assertThat(prodFactList).hasSize(databaseSizeBeforeTest);
    }
    ...
    void putNewProdFact() throws Exception {
        ...
        updatedProdFact.cantidad(UPDATED_CANTIDAD);
        ...
        ProdFact testProdFact = prodFactList.get(prodFactList.size() - 1);
        assertThat(testProdFact.getCantidad()).isEqualTo(UPDATED_CANTIDAD);
    }
    ...
    void fullUpdateProdFactWithPatch() throws Exception {
        ...
        partialUpdatedProdFact.cantidad(UPDATED_CANTIDAD);
